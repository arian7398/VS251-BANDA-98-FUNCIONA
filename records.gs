...
function saveRegistroToCaso(formData) {
  ...
  for (var i = 3; i < headers.length; i++) {
    ...
    // Valor especial para el campo Estado
    if (header === 'Estado') {
      value = 'ACTIVO'; // Establecer el estado inicial como "Activo"
    } else {
      // Buscar valor con diferentes estrategias
      if (formData[header]) {
        value = formData[header];
  ...
  var recordObject = {
    ...
    'Fecha de Registro': fechaRegistro,
    'Usuario Creador': currentUser.toUpperCase(),
    'Estado': 'Activo' // Asegurarse de incluir estado en el objeto devuelto
  };
...
function getCasoById(id) {
  ...
  console.log("getCasoById - ID recibido:", id);
  console.log("Tipo de ID:", typeof id);
  ...
  console.log("Buscando registro para usuario:", currentUser);
  ...
  // Buscar el registro con el ID específico
  for (var i = 0; i < data.length; i++) {
    // Verificar si la primera columna coincide con el ID
    if (String(data[i][0]) === String(id)) {
      console.log("Registro encontrado en fila:", i + 2); // +2 porque i empieza en 0 y la primera fila son encabezados
    ...
    if (data[i][0] === id) {
      ...
      for (var j = 0; j < headers.length; j++) {
        ...
        if (j === 0) { // ID
          ...
          rowData.push(data[i][1]); // Mantener fecha original
        } else if (header === 'Estado') { // Campo Estado
          rowData.push('Actualizado'); // Cambiar a "Actualizado" al editar
        } else {
          // Actualizar si se proporcionó un nuevo valor, sino mantener el original
          rowData.push(formData[header] !== undefined ? formData[header] : data[i][j]);
        }
    ...
    if (String(data[i][0]) === String(id)) {
      ...
      for (var j = 0; j < headers.length; j++) {
        ...
        if (j === 0) { // ID
          ...
          rowData.push('ACTUALIZADO'); // Cambiar a "ACTUALIZADO" al editar (en mayúsculas)
        } else {
          // Actualizar si se proporcionó un nuevo valor, sino mantener el original
          var newValue = formData[header] !== undefined ? formData[header] : data[i][j];
